...
export default function ArtDepartmentVehicleDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          {vehicle.specialFeatures.length > 0 && (
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {vehicle.specialFeatures.map((f) => <Badge key={f}>{f}</Badge>)}
              </div>
            </div>
          )}
        </CardContent>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-center">
        <p className="text-sm text-white/50">To place an option on this vehicle, go to one of your allocated projects.</p>
      </div>
    </div>
  );
}
