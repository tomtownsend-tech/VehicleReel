...
export default function AdminBookingsPage() {
  ...
  if (loading) {
    return <div><h1 className="text-2xl font-bold text-white mb-6">Bookings</h1><div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-16 bg-gray-800 rounded-xl animate-pulse" />)}</div></div>;
  }

  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">Bookings ({bookings.length})</h1>
      {bookings.length === 0 ? (
